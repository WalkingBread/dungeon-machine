from logic.game.session import SessionManager, GameSession
...
class GameService:
    ...
    @validate_session
    def create_character(self, session: GameSession, player_id: str, name: str):
        return session.create_character(
            session.get_player(player_id),
            name
        )
