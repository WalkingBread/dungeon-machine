...
from logic.brain.model.request import StoryUpdate, StoryIntroduction, SceneUpdate
from logic.brain.model.chain.base import BaseLangChainWrapper

GAME_INTRO_SYSTEM_MESSAGE = (
...
class StoryIntroChain(BaseLangChainWrapper):
    ...
    def _compile_chain(self) -> Runnable:
        prompt = ChatPromptTemplate.from_messages([
            {"role": "system", "content": GAME_INTRO_SYSTEM_MESSAGE},
            ("user", "## STORY SETTING:\n{story_theme}"),
            ("user", f"## TASK:\n Generate an engaging, punchy, and atmospheric introduction to kick off a new adventure based upon the provided STORY SETTING.")
        ])
...
class StoryUpdateChain(BaseLangChainWrapper):
    ...
    def _compile_chain(self) -> Runnable:
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("user", "## GAME_STATE:\n{game_state}"),
            ("user", "## CURRENT_PLAYER:\n{player}"),
            ("user", f"## TASK:\n Generate a continuation to the story based on PREVIOUS_SCENE_CONTEXT and GAME_STATE.")
        ])
...
class SceneUpdateChain(BaseLangChainWrapper):
    ...
    def _compile_chain(self) -> Runnable:
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("user", "## GAME_STATE:\n{game_state}"),
            ("user", "## CURRENT_PLAYER:\n{player}"),
            ("user", f"## TASK:\n This is the next player's turn (CURRENT_PLAYER) to act. Introduce him in the situation based on SCENE_CONTEXT.")
        ])
